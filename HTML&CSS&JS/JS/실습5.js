...
let fruits2 = ["복숭아", "수박", "포도", "딸기", "오렌지"]

// for 요소 출력
console.log("== for 요소 출력 ==")
for (let i = 0; i < fruits2.length; i++) {
  console.log(fruits2[i]);
}

// for ...of 요소 출력
console.log("== for ...of 요소 출력 ==")
for (let fruit of fruits2) {
  console.log(fruit); 
}

// forEach 요소 출력
console.log("== forEach 요소 출력 ==")
fruits1.forEach((n) => {
    console.log(n);
});
...
// 오름차순 정렬
console.log("== 배열 조작 ==")
const above_5 = function (nums) {
    let lst = [];

    nums.forEach((num) => {
        if (num > 5) {
            lst.push(num ** 2);  // append → push
        }
    });

    return lst.sort((a, b) => a - b);  // sorted() 없음 → sort(compare)
};

console.log(above_5([3, 7, 2, 9, 5, 1, 8]));
...
const avg_student = function(students) {
    students.forEach(student => {
        let sum = 0;
        student.scores.forEach(score => sum += score);
        let avg = sum / student.scores.length;
        console.log(student.name, avg.toFixed(2)); // 소수 둘째 자리까지
    });
};

avg_student(students);