...
# 2) 훈련:테스트 = 70:30 분할 + stratify 적용
X_train, X_test, y_train, y_test = train_test_split(
    x, y,
    test_size=0.30,       # 30% 테스트
    stratify=y,           # 타깃 클래스 비율 유지
    random_state=42       # 재현성
)

print("전체 샘플 수:", len(y))
print("훈련 샘플 수:", len(y_train))
print("테스트 샘플 수:", len(y_test))

# 3) 클래스 분포 확인(개수/비율)
def show_class_dist(name, labels):
    classes, counts = np.unique(labels, return_counts=True)
    ratios = counts / counts.sum()
    print(f"\n[{name}] 클래스 분포")
    for c, cnt, r in zip(classes, counts, ratios):
        print(f"  class {c}: {cnt}개 ({r:.3f})")

show_class_dist("전체", y)
show_class_dist("훈련", y_train)
show_class_dist("테스트", y_test)
